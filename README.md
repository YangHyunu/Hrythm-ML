...
> 메타 러너로 **LDA 사용 시 최고 성능**을 기록
 - 특이 점은 일반적으로 메타 러너로 로지스틱 회귀모델을 사용하지만 LDA가 가장 좋은 성능을 보였음 , 다만 메타 러너에 QDA를 사용한 경우 베이스 러너들의 하이퍼파라미터 튜닝에 따라 성능이 크게 달라졌음. -> 이 후 오토인코더 를 통해 비선형 특성을 추출한 후 모델을 학습했을 때는 가장 좋은 성능을 보여줬음 
